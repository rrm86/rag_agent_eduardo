...
def process_json_file_semantic(filename: str, breakpoint_type: str = "percentile", 
                              threshold_amount: float = 0.5, min_chunk_size: int = 100) -> List[str]:
    """
    Realiza o parseamento de um arquivo JSON usando o SemanticChunker.
    
    Args:
        filename: Nome do arquivo JSON a ser processado
        breakpoint_type: Tipo de limiar para quebra de chunks ('percentile', 'standard_deviation', 'interquartile', 'gradient')
        threshold_amount: Valor do limiar para quebra de chunks
        min_chunk_size: Tamanho mínimo de cada chunk em caracteres
        
    Returns:
        Lista de documentos divididos semanticamente
    """
    ...
    # Converte o JSON para string para processamento
    ...
    # Divide o texto baseado em similaridade semântica
    docs = semantic_splitter.create_documents([json_str])
    
    # Extrai o conteúdo dos documentos
    doc_contents = [doc.page_content for doc in docs]
    
    print(f"Documento dividido em {len(doc_contents)} chunks semânticos")
    
    return doc_contents


def upload_to_supabase(doc_splits: List[str], collection_name: str = None, clear_collection: bool = False):
    """
    ...
    if collection_name is None:
        collection_name = SUPABASE_COLLECTION_NAME.split(",")[4]
...
def main():
    ...
    # Configurações para o semantic chunker
    breakpoint_types = ["percentile", "standard_deviation", "interquartile", "gradient"]
    selected_type = breakpoint_types[0]  # Usando 'percentile' como padrão
    
    # Definir o nome da coleção
    collection_name = SUPABASE_COLLECTION_NAME.split(",")[4]  # semantic1
    ...
    for filename in json_files:
        doc_splits = process_json_file_semantic(
            filename, 
            breakpoint_type=selected_type,
            threshold_amount=0.5,  # Valor padrão, pode ser ajustado
            min_chunk_size=100     # Tamanho mínimo de chunk, pode ser ajustado
        )
